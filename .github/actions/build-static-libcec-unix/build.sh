...
mkdir -p dynamic-dist/include
mkdir -p static-dist/include

find platform_build \( -o -name '*.so' -o -name '*.dylib' \) -print -exec cp {} dynamic-dist \;
find platform_build/include -name '*.h' -print -exec cp --parents {} dynamic-dist \;
find build \( -o -name '*.so' -o -name '*.dylib' \) -print -exec cp {} dynamic-dist \;
